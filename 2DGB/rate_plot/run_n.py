...
for n in N:
    ...
    for t in range(T):
        try:
            ...
            PRF = np.dot(RF,RF.T)
            Nt.append(Dmetric(PR,PRF,p0))
        except:
            print('SVD Error')
    print(str(n)+' completed.')
    N1.append(np.mean(Nt))
...
sns.regplot(x=np.log(N),y=np.log(N1),label='h=0.2')
sns.regplot(x=np.log(N),y=np.log(N2),label='h=0.3')
sns.regplot(x=np.log(N),y=np.log(N3),label='h=0.4')
...
plt.savefig(r'./output/n.png', bbox_inches='tight')
