...
for p in P:
    ...
    for t in range(T):
        try:
            ...
            PRF = np.dot(RF,RF.T)
            Pt.append(Dmetric(PR,PRF,p0))
        except:
            print('SVD Error')
    print(str(p)+' completed.')
    G1.append(np.mean(Pt))
...
sns.regplot(x=np.log(P),y=np.log(G1),label='n=30')
sns.regplot(x=np.log(P),y=np.log(G2),label='n=60')
sns.regplot(x=np.log(P),y=np.log(G3),label='n=120')
...
plt.savefig(r'./output/p.png', bbox_inches='tight')