...
for h in H:
    ...
    for t in range(T):
        try:
            ...
            PRF = np.dot(RF,RF.T)
            Ht.append(Dmetric(PR,PRF,p0))
        except:
            print('SVD Error')
    print(str(h)+' completed.')
    H1.append(np.mean(Ht))
...
plt.figure(dpi=300,figsize=(3.5, 5)) #设定图像尺寸

sns.regplot(x=np.log(H),y=np.log(H1),label='n=50')
sns.regplot(x=np.log(H),y=np.log(H2),label='n=100')
sns.regplot(x=np.log(H),y=np.log(H3),label='n=200')

plt.ylabel('log(error)')

plt.xlabel('log(h)')

plt.xticks([-1.8,-1.5,-1.2,-0.9,-0.6])
plt.yticks([-4,-3.1,-2.2,-1.3])

plt.legend()
plt.savefig(r'./output/h.png', bbox_inches='tight')